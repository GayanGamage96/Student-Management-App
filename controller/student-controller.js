...
const updateStudent = (req, res) => {
    connection.query('UPDATE student set name=?, address=?, registered_date=?, course=?, image=? WHERE std_id=?', [req.body.name, req.body.address, req.body.registered_date, req.body.course, req.body.image, req.params.id], (err, rows) => {
        if (err) throw err
        res.json(rows)
    })
}
